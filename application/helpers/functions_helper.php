...
if (!function_exists('emailSetting')) {
	function emailSetting()
	{
		...
		$config['smtp_host'] = 'izodiac.me';
		$congig['smtp_user'] = 'no-reply@izodiac.me';
		$congig['smtp_pass'] = '*izodiac13*';
		$congig['smtp_port'] = 465;
		$congig['smtp_timeout'] = 10;
...
if (!function_exists('sub_menu')) {
	...
	{
		$ci = get_instance();
		if ($ci->ModelContents->exists(" WHERE id_content = '".$id_menu."' "))
			return $ci->ModelContents->getRows(" WHERE id_content = '".$id_menu."' ");
		else
			return false;
	}
}
